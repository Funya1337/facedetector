...
import requests
from classification.config import token
import vk
...
def _getMembers(group_id):
    members = vk_api.groups.getMembers(group_id=group_id)
    count = members['count']
    all_members = []
    offset = 0
    while offset < count:
        members = vk_api.groups.getMembers(group_id=group_id, offset=offset)
...
def parseAllUsers(group_ids=["wonder_666"]):
    global progress
    global all_len
